...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // --- 2. MATCH MANAGEMENT (FIXED LOGIC) ---
        if (type === 'get_admin_matches') {
            let sql = 'SELECT * FROM matches ORDER BY match_time DESC LIMIT 50';
            let params = [];
            // ক্যাটাগরি আইডি যদি 'null' বা আনডিফাইন্ড না হয় তবেই ফিল্টার করবে
            ...
            try {
                ...
                for (let p of prizes) {
                    ...
                    if (user.length > 0) {
                        ...
                        await connection.execute('INSERT INTO transactions (user_id, amount, type, details, status, created_at) VALUES (?, ?, "Win", ?, "completed", NOW())', [uid, p.total, `Match #${match_id} Prize`]);
        ...
        if (type === 'handle_withdrawal') {
            if (body.action === 'approve') { await db.execute("UPDATE withdrawals SET status='approved' WHERE id=?", [body.withdraw_id]); }
            else {
